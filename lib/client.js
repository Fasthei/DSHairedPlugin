...
const CSS = [
  ...
  '.rt-card .rt-textarea{min-height:120px}',
  '.rt-card.rt-card-wide .rt-textarea{min-height:180px}',
  ...
  '.rt-card{position:absolute;left:10px;bottom:10px;width:440px;max-height:82%;overflow:auto;background-color:var(--dsw-alias-bg-overlay,rgba(20,22,26,.95));color:var(--dsw-alias-label-primary,inherit);border:1px solid var(--dsw-alias-border-l2,rgba(128,128,128,.45));border-radius:8px;padding:12px;z-index:3;display:flex;flex-direction:column;gap:8px;font-size:12.5px}',
  '.rt-card.rt-card-wide{left:10px;right:10px;width:auto;max-height:88%;font-size:13px}',
...
function applyClient(ctx) {
  ...
  function Panel() {
    ...
    // 详情卡「展开」状态：默认 440px 浮动卡，展开后铺满中间区域（看长 URL / 证据更舒服）
    const [wideCard, setWideCard] = React.useState(false)
    ...
    // 「待验证」是研判阶段的暂定状态：模型拿不准是否同主体时先登记并打这个标签。
    // 人工核实后走这里确认——摘掉标签、写一条确认记录（可追溯是谁在什么时候拍的板）。
    const VERIFY_TAG = '待验证'
    const CONFIRM_TAG = '已确认'
    function needsVerify(a) { return !!(a && Array.isArray(a.tags) && a.tags.indexOf(VERIFY_TAG) >= 0) }
    function confirmAsset(a) {
      if (!a) return Promise.resolve()
      const tags = (a.tags || []).filter(function (t) { return t !== VERIFY_TAG })
      if (tags.indexOf(CONFIRM_TAG) < 0) tags.push(CONFIRM_TAG)
      const stamp = fmtTime(Date.now())
      const note = (a.note ? a.note + '\n' : '') + '【人工确认】' + stamp + '：已核对与目标同主体，取消「待验证」。'
      return commitPatch(a.id, { tags: tags, note: note }).then(function () {
        setInfo('已确认「' + a.value + '」，已移除「待验证」')
      })
    }
        ...
        showList ? React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 4, flex: '1 1 auto', minHeight: 0 } },
          ...
          React.createElement('div', { className: 'rt-list rt-scroll', style: { flex: '1 1 auto', minHeight: 54 } }, filtered.map(function (a) {
            return React.createElement('div', { key: a.id, className: 'rt-item' + (a.id === selected ? ' rt-sel' : ''), onClick: function () { setSelected(a.id) } },
              ...
              needsVerify(a) ? React.createElement('button', {
                className: 'rt-btn rt-primary',
                style: { padding: '0 6px', fontSize: 11, lineHeight: '18px' },
                title: '核实该资产确为同主体后点此确认：移除「待验证」并写入确认记录',
                disabled: !!busy,
                onClick: function (ev) { ev.stopPropagation(); confirmAsset(a) }
              }, '确认') : null)
          }))) : null)
    )

    const detailCard = sel
      ? React.createElement('div', { className: 'rt-card' + (wideCard ? ' rt-card-wide' : '') },
          React.createElement('div', { className: 'rt-row', style: { flexWrap: 'nowrap' } },
            ...
            React.createElement('button', {
              className: 'rt-btn' + (wideCard ? ' rt-on' : ''),
              title: wideCard ? '收起详情卡' : '展开详情卡（铺满中间区域，便于看长 URL 与证据）',
              onClick: function () { setWideCard(!wideCard) }
            }, wideCard ? '收起' : '展开'),
            React.createElement('button', { className: 'rt-btn', title: '关闭详情卡', onClick: function () { setSelected(null) } }, '×')),
          ...
          React.createElement('div', { className: 'rt-row' },
            needsVerify(sel) ? React.createElement('button', {
              className: 'rt-btn rt-primary',
              title: '核实该资产确为同主体后点此确认：移除「待验证」并写入确认记录',
              disabled: !!busy,
              onClick: function () { confirmAsset(sel) }
            }, '确认同主体') : null,
