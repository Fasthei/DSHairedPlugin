...
| **自动捕获** | 从工具输出里自动攒**候选**（不直接入库）。攒到 6 条、或安静 45 秒后自动唤醒模型研判，由模型决定登记哪些 |
...
`自动捕获` 开关打开时，插件仍然从工具输出里提资产，但**只进候选池，不写图谱**：
...
顶栏的「待研判候选 N」是**只读指示**，没有按钮 —— 整个过程自己跑。触发时机挂在客户端每 2 秒的 `snapshot` 心跳上（host 侧没有可用的计时器）。
