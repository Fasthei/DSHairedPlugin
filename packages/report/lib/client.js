...
function applyClient(ctx) {
  ...
  function Panel() {
    ...
    return React.createElement('div', { className: 'rtm-root' },
      React.createElement('div', { className: 'rtm-title' }, '红队报告'),
  ...
  ctx.effect(function () {
    return slots.inject('sidebar.panellist', function () {
      return slots.register({ name: 'sidebar.panellist', id: 'redteam-report', order: 60, label: '红队报告' }, Glyph)
    })
  }, 'redteam-report: panel button')

  ctx.effect(function () {
    return slots.inject('main', function () {
      return slots.register({ name: 'main', key: 'redteam-report' }, Panel)
    })
  }, 'redteam-report: main panel')

  console.log('[rtasset] redteam-report client half ready')
}
