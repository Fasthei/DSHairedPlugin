...
# 本地 tarball
npm pack && dsh plugin --profile web add ./dsh-redteam-asset-graph-7.9.7.tgz
...
**`7.9.7`**（仓库版本）。`src/` 是权威源码，`lib/` 由它生成，因此**不存在「仓库版本与运行版本不一致」**。

本版只修一处**致命**缺陷：`inject` 补上 `'tools'`。三个模型工具都走 `ctx.tools.register`，
而 Cordis 只允许访问 `inject` 里声明过的服务 —— 属性访问未声明的服务会在插件树加载期抛
`cannot get property "tools" without inject`。**`7.9.6` 正是以这种方式发出去的：装了它的
profile 不是少三个工具，是整个 dsh 起不来**，升级到本版即恢复。

同一处闸门也固化进了生成器（`tools/build-lib.mjs`）：src 里注册了模型工具、而 `inject`
缺 `'tools'` 就中止构建 —— `node --check` 与 `--check` 都拦不住这类错，只能靠产物结构断言。

`7.9.5` 的内容（候选池结算 —— 研判结束后把本轮送出、且未被 `asset_record` 采纳的候选出池，
模型这一轮若没有任何推理/输出则把候选退回待研判、重推上限 3 次以防空转；面板「待验证」确认
按钮与详情卡放大/展开；`lib/` 常驻插件包与 npm 分发要件 `dsh.bundle.patch`）见 git log 与
tag `v7.9.5`。
